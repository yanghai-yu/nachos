...
public class AutoGrader {
    /**
     * Allocate a new autograder.分配一个新的自动分级器
     */
    public AutoGrader() {
    }

    /**启动自动分级器，加载-#参数，调用init()，加载并初始化内核，调用run()
     * Start this autograder. Extract the <tt>-#</tt> arguments, call
     * <tt>init()</tt>, load and initialize the kernel, and call
     * <tt>run()</tt>.
     *
     * @param	privilege      	encapsulates privileged access to the Nachos 封装了对Nachos机器的特权访问
     * 				machine.
     */
