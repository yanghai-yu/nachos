...
public class CoffSection {
    ...
    /**
     * 将一个section load到对应的swap页中
     *
     * @param    spn    the page number within this segment.
     */
    public byte[] loadPageToBuffer(int spn) {
        Lib.assertTrue(file != null);
        ...
        if (initlen > 0)
            Lib.strictReadFile(file, faddr, buffer, 0, initlen);//将有用的部份内容装入buffer中

        Arrays.fill(buffer, initlen, pageSize, (byte) 0);//将没有用的部份置为0
